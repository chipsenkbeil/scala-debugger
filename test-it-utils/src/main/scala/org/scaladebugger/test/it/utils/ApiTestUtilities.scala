package org.scaladebugger.test.it.utils

import java.util.concurrent.atomic.AtomicBoolean

import com.sun.jdi.ThreadReference
import com.sun.jdi.event.{BreakpointEvent, StepEvent}
import org.scaladebugger.test.common.utils.ApiConstants
import org.scalatest.concurrent.Eventually
import org.scalatest.concurrent.PatienceConfiguration.{Interval, Timeout}
import org.scalatest.time.{Milliseconds, Span, Units}
import org.scaladebugger.test.common

/**
 * Contains helper methods for testing.
 */
trait ApiTestUtilities extends Eventually { this: Logging =>
  import ApiConstants.EventuallyTimeout
  implicit override val patienceConfig = PatienceConfig(
    timeout = scaled(common.utils.ApiConstants.EventuallyTimeout),
    interval = scaled(common.utils.ApiConstants.EventuallyInterval)
  )
  ...
  def lowlevelVerifyStepsOnEach[T](
    ...
    (s: ScalaVirtualMachine, start: () => Unit, stepMethod: (ThreadReference) => T) => {
      ...
      logTimeTaken(eventually(
        ...
        interval = Interval(scaled(common.utils.ApiConstants.EventuallyInterval))
      ) {
