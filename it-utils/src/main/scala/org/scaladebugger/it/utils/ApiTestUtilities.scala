...
import org.scaladebugger.test.utils.ApiConstants
...
trait ApiTestUtilities extends Eventually { this: Logging =>
  ...
  implicit override val patienceConfig = PatienceConfig(
    timeout = scaled(utils.ApiConstants.EventuallyTimeout),
    interval = scaled(utils.ApiConstants.EventuallyInterval)
  )
  ...
  def lowlevelVerifyStepsOnEach[T](
    ...
    (s: ScalaVirtualMachine, start: () => Unit, stepMethod: (ThreadReference) => T) => {
      ...
      logTimeTaken(eventually(
        ...
        interval = Interval(scaled(utils.ApiConstants.EventuallyInterval))
      ) {
